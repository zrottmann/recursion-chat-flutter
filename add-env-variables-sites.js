...
async function addSiteEnvironmentVariables(siteId) {
  ...
  try {
    ...
    try {
      ...
      console.log(`⚠️  Could not verify site: ${error.message}`);
    }
    ...
    if (successCount > 0) {
      ...
      console.log('🚀 Your Trading Post site should now have the required environment variables.');
...
if (!siteId) {
  console.log('🎯 Usage: node add-env-variables-sites.js <SITE_ID>');
  console.log('📝 Example: node add-env-variables-sites.js 689cb6a9003b47a75929');
  console.log('\n💡 To find your Site ID:');
  console.log('   1. Go to: https://cloud.appwrite.io/console/project-689bdaf500072795b0f6');
  console.log('   2. Navigate to: Hosting → Sites');
  console.log('   3. Click on your Trading Post site');
  console.log('   4. The Site ID is in the URL: /hosting/site-[SITE_ID]');
  process.exit(1);
}

addSiteEnvironmentVariables(siteId);