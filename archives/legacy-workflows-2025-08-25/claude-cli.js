...
async function spawnClaude(command, options = {}, ws) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      console.log(
        `  Claude config exists: ${fsSync.existsSync(claudeConfigPath)}`,
      );
      ...
      if (fsSync.existsSync(claudeConfigPath)) {
        try {
          ...
          if (
            ...
            console.log(
              `✅ Found ${
                Object.keys(claudeConfig.mcpServers).length
              } global MCP servers`,
            );
            hasMcpServers = true;
          }
      ...
      if (hasMcpServers) {
        ...
        if (fsSync.existsSync(claudeConfigPath)) {
          try {
            ...
            const hasGlobalServers = claudeConfig.mcpServers &&
              Object.keys(claudeConfig.mcpServers).length > 0;
            const currentProjectPath = process.cwd();
            const projectConfig = claudeConfig.claudeProjects &&
              claudeConfig.claudeProjects[currentProjectPath];
      ...
      console.log(
        "Note: MCP config check failed, proceeding without MCP support",
      );
    }
