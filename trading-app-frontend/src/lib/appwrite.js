...
// Enhanced error handler with CORS detection and proper error object handling
export const handleAppwriteError = (error) => {
  ...
  if (error.code && errorMessages[error.code]) {
    ...
    // Handle specific Appwrite error patterns
    const errorMsg = error.message.toLowerCase();
    
    if (errorMsg.includes('invalid credentials') || errorMsg.includes('invalid email') || errorMsg.includes('wrong credentials')) {
      message = 'Invalid email or password. Please check your credentials and try again.';
    } else if (errorMsg.includes('user not found')) {
      message = 'No account found with this email address. Please check your email or create a new account.';
    } else if (errorMsg.includes('email already exists') || errorMsg.includes('user already exists')) {
      message = 'An account with this email already exists. Please log in instead or use a different email.';
    } else if (errorMsg.includes('password')) {
      message = 'Password does not meet requirements. Please check the password format.';
  ...
  if (!message || message.trim() === '' || message === '[object Object]') {
