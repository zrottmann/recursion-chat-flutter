...
module.exports = async ({ req, res, log, error }) => {
  ...
  try {
    ...
    if (!apiKey) {
      log('⚠️ No API key found, will use fallback responses');
    } else {
      log(`🔑 API Key configured: Yes (length: ${apiKey.length})`);
    }
    ...
    if (apiKey) {
      ...
      log('📌 Skipping API call - no API key configured');
      xaiError = 'No API key configured';
    }
